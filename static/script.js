...
const CONFIG = {
  mobile: {
    minBudget: 5000, maxBudget: 150000, step: 1000, defBudget: 20000,
    presets: [10000, 20000, 30000, 50000, 80000],
    priorities: [
      ...
      { id: "value", icon: "💎", name: "Value for Money", desc: "Best overall specs per rupee" },
      { id: "5g", icon: "⚡", name: "5G Ready", desc: "Future-proof connectivity" },
    ],
    brands: ["Xiaomi", "Samsung", "OnePlus", "Apple", "Realme", "Nothing", "Google", "Motorola", "iQOO", "Vivo", "POCO"]
  },
  laptop: {
...
(function initParticles() {
  ...
  function mkParticle() { return { x: Math.random() * W, y: Math.random() * H, r: Math.random() * 1.5 + 0.3, vx: (Math.random() - 0.5) * 0.2, vy: (Math.random() - 0.5) * 0.2, color: ["#6c63ff", "#a78bfa", "#38bdf8", "#00f5c4"][Math.floor(Math.random() * 4)] }; }
...
function setupBudgetStep() {
  ...
  slider.min = conf.minBudget;
  slider.max = conf.maxBudget;
  slider.step = conf.step;
  slider.value = state.budget = conf.defBudget;
  
  // Update presets
  const presetsArea = document.getElementById("budget-presets");
  presetsArea.innerHTML = conf.presets.map(p => 
    `<button class="preset-chip" onclick="setPresetBudget(${p}, this)">₹${(p/1000)}K</button>`
  ).join("");
...
function updateBudget() {
  state.budget = parseFloat(document.getElementById("budget-slider").value);
  updateBudgetDisplay();
  document.querySelectorAll(".preset-chip").forEach(c => c.classList.remove("active"));
}

function setPresetBudget(val, btn) {
  state.budget = val;
...
function updateBudgetDisplay() {
  document.getElementById("budget-display").textContent = `₹ ${state.budget.toLocaleString("en-IN")}`;
}
...
function setupPriorityStep() {
  ...
  grid.innerHTML = conf.priorities.map(p => `
...
async function runRecommend() {
  ...
  try {
    ...
    if (data.error) {
      document.getElementById("results-list").innerHTML = `<div class="empty-state">❌ Error: ${data.error}</div>`;
      return;
    }
...
function renderResults(recs) {
  ...
  const ranks = [
    { label: "🥇 First Choice",     class: "rank-1", color: "#ffd700" },
    { label: "🥈 Excellent Match", class: "rank-2", color: "#c0c0c0" },
    { label: "🥉 Great Value",     class: "rank-3", color: "#cd7f32" }
  ];
  ...
  recs.forEach((rec, i) => {
    ...
    const geminiHtml = rec.gemini_explanation ? `
      <div class="gemini-box">
        <div class="gemini-box-hdr">⚡ AI Insight</div>
        <p>${rec.gemini_explanation}</p>
      </div>` : "";

    const chartId = `radar-${i}`;
    const ptags = state.priorities.size > 0 ? 
      Array.from(state.priorities).map(p => {
        const pobj = CONFIG[state.device].priorities.find(x => x.id === p);
        return pobj ? `<span class="ptag">${pobj.icon} ${pobj.name}</span>` : "";
      }).join("") : `<span class="ptag">⚖️ Excellent All-rounder</span>`;

    const card = document.createElement("div");
    card.className = `result-card`;
    card.style.animationDelay = `${i * 0.15}s`;
    
    // Add live AI badge if its from Gemini
    const liveBadge = rec.is_live_gemini ? `<span class="live-badge">✨ LIVE AI PICK</span>` : "";

    card.innerHTML = `
      <div class="card-rank-strip">
        <div class="rank-badge">
          <span class="rank-emoji">${r.label.split(" ")[0]}</span>
          <span class="rank-label">${r.label.split(" ").slice(1).join(" ")} ${liveBadge}</span>
        </div>
        <div class="card-score-pills">
          ...
          <div class="score-pill conf">
            <span class="score-num">${rec.confidence}%</span>
            <span class="score-lbl">AI Conf.</span>
          </div>
        </div>
      </div>
      ...
      <div class="card-body">
        <div class="card-left">
          ...
          <div class="match-bar-wrap">
            <div class="match-bar-label"><span>Overall AI Compatibility</span><span>${rec.score}%</span></div>
            <div class="match-bar-bg"><div class="match-bar-fill" id="bar-${i}" style="width:0%"></div></div>
          </div>
        ...
        <div class="card-right">
          <div class="radar-wrap"><canvas id="${chartId}"></canvas></div>
          <div class="radar-title">Spec Balance</div>
        </div>
      </div>
      
      <div class="card-footer">
        <div class="priority-tags">Optimised for: ${ptags}</div>
        <button class="buy-btn" onclick="window.open('https://www.amazon.in/s?k=${encodeURIComponent(rec.name)}', '_blank')">Find on Amazon ↗</button>
      </div>
    `;
...
function drawRadar(id, data, idx) {
  ...
  if (!ctx || !data) return;
  const colors = [
    ["rgba(108,99,255,0.7)", "rgba(108,99,255,0.2)"],
    ["rgba(56,189,248,0.7)", "rgba(56,189,248,0.2)"],
    ["rgba(167,139,250,0.7)", "rgba(167,139,250,0.2)"],
  ][idx % 3];
  ...
  state.radarCharts[id] = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: data.values,
        borderColor: colors[0], backgroundColor: colors[1], borderWidth: 1.5,
        pointBackgroundColor: colors[0], pointBorderColor: "transparent", pointRadius: 2,
      }]
    },
    options: {
      ...
      scales: {
        r: {
          ...
          ticks: { display: false },
          pointLabels: { color: "#9aabcf", font: { size: 8.5, family: "Inter" } }
        }
      }
    }
  });
...
async function loadMarketInsight() {
  if (state.insightLoaded) {
    document.getElementById("insight-panel").classList.toggle("hidden");
    return;
  }
  ...
  try {
    const res = await fetch("/market-insight", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ category: state.device, budget: state.budget })
    });
    const data = await res.json();
    ...
    content.innerHTML = `
      ...
      <div style="margin-bottom:12px">
        <div class="insight-label">🔥 Trending Now</div>
        <div class="insight-chips">${(data.hot_picks || []).map(p => `<span class="insight-chip">${p}</span>`).join("")}</div>
      </div>
      <div class="insight-warning">⚠️ ${data.avoid || ""}</div>
      <div class="insight-protip">💡 Pro Tip: ${data.pro_tip || ""}</div>
    `;
    state.insightLoaded = true;
  } catch(e) {
    loader.style.display = "none";
    content.innerHTML = `<div style="color:var(--accent-r)">Error loading insight.</div>`;
  }
}
...
function exportPDF() {
  ...
  doc.text("NeuraFind — AI Device Match", 20, y); y += 10;
  ...
  state.results.forEach((rec, i) => {
    ...
    doc.text(`Match Score: ${rec.score}% | AI Confidence: ${rec.confidence}%`, 24, y); y += 6;
    
    doc.setTextColor(154, 171, 207);
    Object.entries(rec.specs).forEach(([k, v]) => { doc.text(`• ${k}: ${v}`, 24, y); y += 5; });
    
    if (rec.gemini_explanation) {
      doc.setFont("helvetica", "italic"); doc.setTextColor(167, 139, 250);
      const lines = doc.splitTextToSize(`AI Insight: ${rec.gemini_explanation}`, 160);
      lines.forEach(l => { doc.text(l, 24, y); y += 5; });
  ...
  try {
    ...
    container.innerHTML = data.history.map((h, i) => `
      <div class="history-card" style="animation-delay:${i*0.1}s">
        <div class="history-icon">${DEVICE_ICONS[h.device_type] || "⚡"}</div>
        <div class="history-info">
          <div class="history-name">Top Pick: ${h.top_result}</div>
          <div class="history-meta">₹${parseInt(h.budget).toLocaleString("en-IN")} · ${h.priorities.join(", ") || "No specific priorities"}</div>
        </div>
...
async function sendChat() {
  ...
  try {
    ...
    removeTyping(); appendChat("bot", "Network error.");
  }
}
...
function appendTyping() {
  const c = document.getElementById("chat-messages");
  const d = document.createElement("div"); d.id = "chat-typing";
  d.className = "chat-msg bot chat-typing";
  d.innerHTML = `<div class="msg-bubble"><div class="dot-dot"></div><div class="dot-dot"></div><div class="dot-dot"></div></div>`;
...
function openBattleMode() {
  ...
  const resA = state.results[0];
  const resB = state.results[1];

  document.getElementById('v-title-1').textContent = resA.name;
  document.getElementById('v-title-2').textContent = resB.name;
  
  // Build specs table
  const specsDiv = document.getElementById('battle-specs-list');
  const allKeys = Object.keys(resA.specs); // Assuming same keys for both
  
  specsDiv.innerHTML = allKeys.map(key => `
    <div class="spec-battle-row">
      <div class="spec-battle-label">${key}</div>
      <div class="spec-battle-values">
        <div class="spec-val a">${resA.specs[key] || 'N/A'}</div>
        <div class="spec-sep"></div>
        <div class="spec-val b">${resB.specs[key] || 'N/A'}</div>
      </div>
    </div>
  `).join("");
  ...
  battleChartInstance = new Chart(ctx, {
    type: "radar",
    data: {
      labels: resA.radar.labels,
      datasets: [
        {
          label: resA.name,
          data: resA.radar.values,
    ...
    options: {
      ...
      scales: {
        r: {
          ...
          ticks: { display: false },
          pointLabels: { color: "#9aabcf", font: { size: 10, family: "Inter" } }
        }
      }
    }
  });
